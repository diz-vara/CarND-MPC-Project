 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int i = 0; i < N; ++i) {
      ...
      //fg[0] += 10. / (vars[v_start + i] + 0.1);
      fg[0] += 0.1*CppAD::pow(vars[v_start + i] - v_ref, 2); ////exp(-0.1*vars[v_start + i]); // -80, 2);
      //fg[0] += 10*CppAD::exp(-0.02*vars[v_start + i]); // -80, 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // degrees (values in radians).
   for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -deg25_in_rad;
    vars_upperbound[i] = deg25_in_rad;
  }
